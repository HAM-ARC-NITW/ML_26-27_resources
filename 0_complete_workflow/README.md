...
## Stage 6: Communicate & Deploy

- Summarise findings for non-technical stakeholders: insights, metric, limitations.
- Save the full pipeline (`joblib.dump(pipeline, "model.pkl")`).
- Deploy as an API or batch job (Flask, FastAPI, Streamlit).
- Monitor for **data drift** and performance decay, and retrain when needed.

---

## Working Example: Titanic

`pipeline_example.py` implements Stages 3 to 5 on the Titanic dataset, using a scikit-learn `Pipeline` so that all preprocessing is fitted on training data only.
...
|-------|-------------------|
| Logistic Regression | 0.859 ± 0.023 |
| Random Forest | 0.855 ± 0.033 |

Random Forest on the held-out test set: accuracy 0.80, ROC-AUC 0.841.
...
| Pitfall | Fix |
|---------|-----|
| **Data leakage** (fitting scaler/imputer on all data, using future info) | split first; use `Pipeline` |
| Skipping the baseline | always compare to a simple model |
| Picking the metric after seeing results | define it in Stage 1 |
| Judging on accuracy with imbalanced data | use precision, recall, F1, PR-AUC |
| Tuning on the test set | tune with CV; test once |
| Deleting outliers blindly | investigate first |
| Confusing correlation with causation | be careful with conclusions |
| Not documenting decisions | keep a log of every cleaning step |

## Quick Checklist

- [ ] Problem, target, and metric defined
- [ ] Data overview and quality checked
- [ ] Distributions, outliers, and relationships explored
- [ ] Train/test split done **before** any fitting
- [ ] Missing values, encoding, and scaling handled in a pipeline
- [ ] Baseline model built
- [ ] Multiple models compared with cross-validation
- [ ] Final model evaluated once on the test set
- [ ] Errors and feature importance analysed
